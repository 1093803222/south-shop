...
exit("请联系TPshop官网客服购买高级版支持此功能");
    ...
    public function edit_order(){
       	...
       	if(IS_POST)
        {
            ...
            if($o && $l){
            	$this->success('修改成功',U('Seller/Order/editprice',array('order_id'=>$order_id)));
            }else{
            	$this->success('修改失败',U('Seller/Order/detail',array('order_id'=>$order_id)));
            }
            exit;
        }
        // 获取省份
    ...
    public function deliveryHandle(){
		...
		if($res){
			$this->success('操作成功',U('Seller/Order/delivery_info',array('order_id'=>$data['order_id'])));
		}else{
			$this->success('操作失败',U('Seller/Order/delivery_info',array('order_id'=>$data['order_id'])));
		}
    }
    ...
    public function refund_balance(){
		...
		$this->ajaxReturn(['status'=>1,'msg'=>"操作成功",'url'=>U("Seller/Order/return_list")]);
    ...
    public function add_return_goods()
   {
    ...
    public function ajax_return_list(){
        ...
        $show = $Page->show();
        $list = M('return_goods')->where($where)->alias('a')->join('order', 'order.order_id=return_goods.order_id')->order("$order_by $sort_order")->limit("{$Page->firstRow},{$Page->listRows}")->select();
        var_dump($list);die;
        ...
        if(IS_POST)
        {
           ...
           $order['total_amount']   = $result['result']['total_amount']; // 订单总价
            ...
            if($order_id)
            {
                ...
                $this->success('添加商品成功',U("Seller/Order/detail",array('order_id'=>$order_insert_id)));
                exit();
            }
            else{
                $this->error('添加失败');
