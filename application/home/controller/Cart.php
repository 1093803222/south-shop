...
class Cart extends Base {
    ...
    public function cart3(){
        ...
        if ($_REQUEST['act'] == 'submit_order') {
            ...
            if(!empty($goods_id))
                $admin_id = M('Goods')->where("goods_id", $goods_id)->getField('admin_id');
            $orderLogic = new OrderLogic();
            $orderLogic->setAction($action);
            $orderLogic->setCartList($order_goods);
            $result = $orderLogic->addOrder($this->user_id,$address_id,$shipping_code,$invoice_title,$coupon_id,$car_price,$user_note,$pay_name,$admin_id); // 添加订单
